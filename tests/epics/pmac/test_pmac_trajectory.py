...
async def test_pmac_build_trajectory(sim_motors: tuple[PmacIO, Motor, Motor]):
    ...
    assert await pmac_io.trajectory.profile_cs_name.get_value() == "CS1"
    assert pmac_trajectory.scantime == 4400000
    assert (
        await pmac_io.trajectory.time_array.get_value()
        == [
            200000.0,
            1000000.0,
            1000000.0,
    ...
    assert (
        await pmac_io.trajectory.positions[7].get_value() == [-1.0, 1, 3, 5, 7, 7.2]
    ).all()

    assert (
        await pmac_io.trajectory.velocities[7].get_value()
        == [
            2.0,
            2.0,
            2.0,
