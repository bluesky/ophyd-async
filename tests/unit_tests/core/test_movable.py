...
class StandardMovableImpl(StandardMovable[float]):
    ...
    @cached_property
    def standard_logic(self) -> MovableLogic[float]:
...
def test_movable_logic_is_cached(movable: StandardMovableImpl):
    logic = movable.standard_logic
    logic2 = movable.standard_logic

    assert logic == logic2


async def test_movable_check_value(movable: StandardMovableImpl):
    movable.standard_logic.check_move = AsyncMock()
    await movable.check_value(5)
    movable.standard_logic.check_move.assert_awaited_once_with(5)
...
async def test_movable_set_calls_movable_logic_check_move_and_calculate_timeout(
    ...
    mock_check_move = movable.standard_logic.check_move = AsyncMock()
    timeout = 5
    mock_calculate_timeout = movable.standard_logic.calculate_timeout = AsyncMock(
        return_value=timeout
    )
    mock_move = movable.standard_logic.move = AsyncMock()
