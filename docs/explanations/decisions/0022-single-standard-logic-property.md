# 22. One `standard_logic` property per Device
...
Each mix-in declares the **same** attribute name, `standard_logic`, with **its own**
required type:
...
- Forgetting the logic now fails with `TypeError: Can't instantiate abstract class ...
  with abstract method standard_logic` rather than an `AttributeError` on `None`.
- `movable_logic` and `flyable_logic` are removed rather than deprecated. A shim would
  ...
  `movable_logic` would override the shim and leave `standard_logic` unimplemented, so the
