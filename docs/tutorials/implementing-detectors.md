...
This behaviour allows us to scale up the framerate of the detector without scaling up the number of documents emitted: whether the detector goes at 10Hz or 10MHz it still only emits one stream datum per stream per flush period, just with different numbers in the `indices` field.
...
Now let's look at the underlying classes that define the detector behaviour:
...
- `prepare()` takes a [](#TriggerInfo) which details everything the detector needs to know about the upcoming acquistion. In this case we just store it for later use.
...
It's job is to control the file writing process on the detector, which may or may not be coupled to the acquisition process:
