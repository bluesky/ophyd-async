"""Tango device servers backing the `ophyd_async.tango.demo` tutorial Devices.
...
client connection machinery, needed instead by the declarative `DemoMotor`/
`DemoStage`/`DemoPointDetector`/`DemoPointDetectorChannel` Devices elsewhere in
this package) and nothing from `ophyd_async.testing` (which pulls in
`bluesky`/`event_model`/`pytest`).

All `GreenMode.Asyncio` (unlike `ophyd_async.tango.testing._tango`'s test-only
devices, which use the default sync green mode) - PyTango only allows one green
mode per device server process, which is why these live in a separate module/
process from the test-only ones rather than sharing `_tango_device_servers.py`.

Served together, at fixed, predictable names, by `_tango_device_servers.py` in
this package - see that module for how to actually run them up.
