...
class EigerDriverIO(Device):
    def __init__(self, prefix: str, name: str = "") -> None:
        self.bit_depth = epics_signal_r(
            int, f"{prefix}BitDepthReadout"
        )  # Should be enum
        self.stale_parameters = epics_signal_r(bool, f"{prefix}StaleParameters")
        self.state = epics_signal_r(str, f"{prefix}DetectorState")  # Should be enum
        self.roi_mode = epics_signal_rw_rbv(str, f"{prefix}RoiMode")  # Should be bool
